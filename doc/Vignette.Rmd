...
Let $\mathbf{c}_{n}$ be the table assigned to the $n$th customer, and $m_k$ is the number of customers sitting at table $k$.

$$P\left(c_{n}=k \mid \mathbf{c}_{1: n-1}\right) \propto\left\{\begin{array}{ll}
\frac{m_{k}}{n-1+\alpha} & \text { if } k \text { is a previously occupied table} \\
...
1. Show that the expected number of tables is
2. Prove exchangeability of the Chinese Random Process
